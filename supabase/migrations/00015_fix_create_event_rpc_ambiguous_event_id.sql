-- Fix ambiguous reference to `event_id` in create_event_with_initial_khatm.
-- The function's OUT column name (`event_id`) can collide with SQL column
-- references inside PL/pgSQL on some Postgres settings/versions.
...
BEGIN
  ...
  RETURN QUERY
  SELECT v_event_id AS event_id, p_short_code AS short_code;
END;
$$;
